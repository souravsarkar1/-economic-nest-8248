...
const KidsJeans = () => {
    ...
    const fetchandRendering = (page, filterby) => {
      setLoading(true);
      axios
        .get(`https://dieselbackend.onrender.com/kidjeans?_page=${page}&_limit=6&_sort=seeling_price&_order=${filterby}`)
        .then((res) => {
  ...
  return (
    <div>
    <Heading>Kids Jeans</Heading>
    <Sortproducts filterby={filterby} setFilterBy={setFilterBy}/>
      <Grid templateColumns='repeat(3, 1fr)' gap={6}>
    {data.map((el)=>(
        <NormalCard key={el.id} {...el} link="kidjeans"/>
    ))}
    </Grid>
